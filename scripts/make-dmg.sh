#!/usr/bin/env bash
# Puts dist/Killdeer.app into dist/Killdeer_<version>_universal.dmg.
#
...
DMG="dist/Killdeer_${VERSION}_universal.dmg"
...
STAGING=$(mktemp -d)
# hdiutil failing here would otherwise leave a whole copy of the app in /var.
...
rm -f "$DMG"
hdiutil create -volname "Killdeer" -srcfolder "$STAGING" -ov -format UDZO "$DMG"
