...
8. **Chrome インスタンスの一覧**: 起動中の Chromium 系ブラウザを、プロファイル・
   user-data-dir・headless 有無・デバッグポート・起動元つきでインスタンス単位に出す。
   AI エージェントに自動操作をさせると同種のブラウザが複数立ち、どれに拡張機能が
   繋がっているか分からなくなるため。CLI が全項目を出し、メニューは1行の要約を出す
...
## 技術的注意点
- **ブラウザの同定**: プロセス名ではなく実行パス中の `.app` バンドル名で判定する。Electron アプリが
  同じ Chromium ヘルパーを抱えており、名前で判定するとエディタのヘルパーを Chrome に数えてしまう。
  実行パスは `argv[0]` ではなくカーネルが記録したものを `KERN_PROCARGS2` から読む
  (`argv[0]` は親が渡した文字列にすぎず、詐称できる)。ブラウザ本体は `Frameworks/` 配下に
  居ないことでヘルパーと区別する (`chrome_crashpad_handler` は `--type=` を持たないため、
  引数だけでは本体と見分けがつかない)
- **crashpad ハンドラの帰属**: Crashpad は二重 fork でハンドラを launchd 直下に置くため、
  親をたどってもブラウザに届かない。祖先たどりだけで孤児と判定すると、**稼働中のブラウザ 1 つにつき
  必ず偽の孤児行が 1 つ出る**。`--database=<dir>/Crashpad` で帰属させるが、これだけでは足りない:
  `--user-data-dir` を指定した Chrome もクラッシュは既定ディレクトリに書くため、
  使い捨てプロファイルを見分けたいという本来の用途でちょうど一致しない。その場合は
  起動元のインストール (`.app` バンドル) で決める。同じインストールから複数のブラウザが
  動いている時はハンドラの帰属先を決められないので、どのインスタンスにも入れない
  (当てずっぽうで片方に入れない)
- **既定プロファイル**: `--profile-directory` が無い時に `Default` と決め打ちしない。Chromium は
  最後に使ったプロファイルを開き直すので、`Local State` の `profile.last_used` を読む
  (このキーを書かないブラウザもあり、その時だけ `Default` にフォールバックする)
